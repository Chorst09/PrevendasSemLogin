...
const VMCalculator: React.FC = () => {
  ...
  const saveProposal = () => {
    ...
    console.log('Nome da proposta:', currentProposal.name);
    console.log('VMs na proposta:', currentProposal.vms.length);

    if (!currentProposal.name) {
      console.log('Erro: Nome da proposta está vazio');
      alert('Por favor, insira um nome para a proposta');
      return;
    }

    if (!currentProposal.clientName) {
      console.log('Erro: Nome do cliente está vazio');
    ...
    console.log('Proposta a ser salva:', proposal);
  ...
  const DREAnalysis: React.FC<{
    ...
    const pieChartData = [
      { label: 'Custo', value: dre.totalCost, percentage: ((dre.totalCost / dre.grossRevenueWithDiscount) * 100), color: 'bg-blue-500' },
      { label: 'Comissões', value: dre.commissions, percentage: ((dre.commissions / dre.grossRevenueWithDiscount) * 100), color: 'bg-orange-500' },
      { label: 'Impostos', value: dre.taxesOnRevenue, percentage: ((dre.taxesOnRevenue / dre.grossRevenueWithDiscount) * 100), color: 'bg-red-500' },
      { label: 'Lucro', value: dre.netProfit, percentage: ((dre.netProfit / dre.grossRevenueWithDiscount) * 100), color: 'bg-green-500' }
    ];
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            value={currentProposal.proposalNumber || generateProposalNumber()}
                            disabled
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setCurrentProposal(prev => ({
                              ...prev,
          ...
          {activeTab === 'edit' && (
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    <span>Editando: {currentProposal.proposalNumber} - {currentProposal.name}</span>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div>
                      <h4 className="font-semibold mb-4">VMs da Proposta</h4>
                      <div className="space-y-3">
                        {currentProposal.vms.map((vm) => (
                          <div key={vm.id} className="border rounded-lg p-4 bg-gray-50">
                            <div className="flex items-center justify-between mb-3">
                              <h5 className="font-medium">{vm.name}</h5>
                              <Button
                                variant="outline"
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm">
                              <div>
                                <span className="text-gray-600">vCPU:</span> {vm.vcpu}
                              </div>
                              <div>
                                <span className="text-gray-600">RAM:</span> {vm.ram}GB
                              </div>
                              <div>
                                <span className="text-gray-600">Storage:</span> {vm.storageSize}GB {vm.storageType}
                              </div>
                              <div>
                                <span className="text-gray-600">Rede:</span> {vm.networkCard}
                              </div>
                              <div>
                                <span className="text-gray-600">OS:</span> {vm.os}
                              </div>
                              <div>
                                <span className="text-gray-600">Qtd:</span> {vm.quantity}
                              </div>
                            </div>
